...
# =================== PREDICTION ===================
def make_prediction(model, reducer, X, input_data):
    try:
        ...
        if hasattr(model, 'predict_proba'):
            return model.predict_proba(input_transformed)[0][1]
        else:
...
def main():
    ...
    try:
        ...
        if df_clean is None or df_clean.empty:
            st.error("Data cleaning failed. Please check your dataset.")
            return
    ...
    with tab4:
        ...
        if os.path.exists("models/model.joblib"):
            try:
                ...
                if submitted:
                    risk = make_prediction(model, reducer, X, input_data)
                    if risk is not None:
